...
func main() {
	...
	if *help {
		...
		fmt.Printf("Usage: \n   Run './chat-with-mdns'\nor Run './chat-with-mdns -host [host] -port [port] -location [location] -rendezvous [string] -pid [proto ID]'\n")

		os.Exit(0)
	}

	fmt.Printf("[*] Listening on: %s with port: %d on location coordinates %s\n", cfg.listenHost, cfg.listenPort, cfg.location)
